  ...
  sysTimeDopar <- system.time({
    for (lng in 1:iterNG) {
      ...
      doparCollect <- foreach (
        ...
        nonmediaAdstocked <- dt_modAdstocked[,(setdiff(names(dt_modAdstocked),set_mediaVarName)), with = F]
        nonmedia_names <- setdiff(names(dt_modAdstocked),set_mediaVarName)
        range01 <- function(x){(x-min(x))/(max(x)-min(x))}
        nonmediaScaled <- nonmediaAdstocked
        for (z in 1:length(nonmedia_names)) {
          nonmediaScaled[[z]] <- range01(nonmediaScaled[[z]])}
        dt_modAdstocked[, (nonmedia_names) := nonmediaScaled]
